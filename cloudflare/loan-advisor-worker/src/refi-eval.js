// Refinance evaluation: break-even, verdict, plain-English bullets.

import { estimateRate, ltvPercent } from './rate-pricing.js';
...
export function termYearsFromLabel(term) {
  const m = String(term || '').match(/(\d+)/);
  return m ? Number(m[1]) : 30;
}

export function monthlyPI(loanAmt, annualRatePct, years = 30) {
  const n = years * 12;
...
export function breakEvenMonths(monthlySavings, closingCosts) {
...
/**
 * @param {object} profile — borrower + loan inputs
 * @param {object} [thresholds]
 * @param {object} [ratesOverride] — optional { base, mid, high } instead of pricing engine
 */
export function evaluateRefi(profile, thresholds = DEFAULT_THRESHOLDS, ratesOverride = null) {
  ...
  if (refiCosts > 0 && isFinite(beMonths)) {
    bullets.push(
      isFinite(beMonths)
        ...
        : `With ${fmtMoney(refiCosts)} in costs, break-even is not reached at this savings level.`
    );
  } else if (refiCosts <= 0) {
...
export function buildAiPrompt(result, profile) {
