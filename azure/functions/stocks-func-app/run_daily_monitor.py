...
# local AI scorer (shared util, no HTTP)
...
EMAIL_INCLUDE_FULL = (os.getenv("EMAIL_INCLUDE_FULL", "0") == "1")  # default OFF (just lists)
EMAIL_SIMPLE = True  # always simple (lists) here; leave flag for future

# How many leaders to force-include in “Stock Picks”
ADD_LEADERS_TOPK = int(os.getenv("ADD_LEADERS_TOPK", "5"))

# Optional MC/HMM gating for buy_flag
USE_MC_HMM_FILTER = (os.getenv("USE_MC_HMM_FILTER", "0") == "1")
MC_MIN_PUP = float(os.getenv("MC_MIN_PUP", "0.55"))       # require >= 55% 30d up-prob
HMM_MIN_BULL = float(os.getenv("HMM_MIN_BULL", "0.50"))   # require >= 50% bull regime probability
...
# --------- OPTIONAL: Monte-Carlo (GBM) + HMM regime ------------------
def _mc_paths_prob_up(last_price: float, mu_d: float, sigma_d: float,
                      horizon_days: int = 30, n_paths: int = 5000,
                      up_threshold: float = 0.0, seed: int | None = 42) -> float:
    """
    Simple GBM Monte Carlo: P(S_T / S_0 - 1 >= up_threshold) over horizon_days.
    mu_d, sigma_d are DAILY drift & vol. Returns probability in [0..1].
    """
...
def _fit_hmm_regime(daily_ret: pd.Series, n_states: int = 2):
    """
    Lightweight HMM regime classification (optional). Requires hmmlearn.
    Returns (state_today, prob_bull) where 'bull' is state with lower variance.
    """
    try:
        from hmmlearn.hmm import GaussianHMM
    except Exception:
        return np.nan, np.nan  # library not installed
    ...
    model.fit(r)
    # identify bull as state with smaller variance
...
def _compute_quarterly_trends(ticker: str) -> Dict[str, float]:
    """
    Build last-4-quarter revenue & earnings trends from quarterly_financials.
    """
    out = {
...
def send_email_report(
    ...
    *,
    ...
    # Build simple lists
    picks = df_all[df_all.get("buy_flag", False) == True].copy()

    # Force-include top leaders (by strength_score) into picks list (dedup).
    leaders_top = df_leaders.sort_values("strength_score", ascending=False).head(ADD_LEADERS_TOPK)
    picks_tickers = list(dict.fromkeys([*picks["ticker"].astype(str).tolist(),
                                        *leaders_top["ticker"].astype(str).tolist()]))

    # AI lists (tickers only)
    def _take_tickers(df, n=None):
        if df is None or df.empty:
            return []
        d = df.copy()
        if "ai_score" in d.columns:
    ...
    ai_spreads_list = _take_tickers(ai_spreads_df, ai_k)
    ai_leaps_list   = _take_tickers(ai_leaps_df,   ai_k)

    # Email subject highlights first 2 names from each AI bucket
    ...
    html_body = f"""<html><body>
      ...
      <div>{_render_list_plain(picks_tickers, 100)}</div>

      <h3>AI: 30–40 Day Debit Call Spreads — Top {ai_k}</h3>
      <div>{_render_list_plain(ai_spreads_list, 100)}</div>

      <h3>AI: LEAPS (12–24 months) — Top {ai_k}</h3>
      <div>{_render_list_plain(ai_leaps_list, 100)}</div>
    """

    # Optional: attach full diagnostic block if you flip EMAIL_INCLUDE_FULL=1
    if EMAIL_INCLUDE_FULL:
        try:
            # quick best-5 tables for context (still small)
            best5 = df_all.sort_values("final_rank", ascending=False).head(5)
            leaps5 = df_leaps.head(5)
            def _tbl(d, cols): 
                d = d[cols].copy()
                for c in d.columns: d[c] = pd.to_numeric(d[c], errors="ignore")
                return d.to_html(index=False, border=0, justify="left")
            html_body += f"""
            <hr>
            <h3>Best 5 Overall (by Final Rank)</h3>
            {_tbl(best5, ["ticker","final_rank","score","strength_score","last_price"])}

            <h3>LEAP Picks (Model, Top 5)</h3>
            {_tbl(leaps5, ["ticker","leap_rank","leap_score","ret_63","ret_252","market_cap"])}
            """
        except Exception:
            pass
...
def run_monitor(tickers: List[str], *, today=None, min_dollar_vol=MIN_DOLLAR_VOL_DEFAULT):
    ...
    for t, df in frames.items():
        ...
        # ---- Monte Carlo: up probability over 30d/40d (GBM) ----
        mu_d    = float(d["ret"].rolling(63).mean().iloc[-1])     # 3m daily drift
        sigma_d = float(d["ret"].rolling(20).std().iloc[-1])      # 1m daily vol
        latest["mc_p_up_30d"] = _mc_paths_prob_up(latest["last_price"], mu_d, sigma_d, 30, 5000, up_threshold=0.0)
        latest["mc_p_up_40d"] = _mc_paths_prob_up(latest["last_price"], mu_d, sigma_d, 40, 5000, up_threshold=0.0)

        # ---- HMM regime (optional; NaN if hmmlearn not installed) ----
        state_today, prob_bull = _fit_hmm_regime(d["ret"])
    ...
    # ---- Single source of truth for "score": use debit_call_spread strategy ----
    out["score"] = out.apply(lambda r: score_row(r, min_dollar_vol, "debit_call_spread"), axis=1)

    # buy_flag based on that score (+ optional MC/HMM filter)
    base_buy = (
        (out["score"] > 1.5) &
    ...
    eps_avg  = out["eps_surprise_avg"].astype(float).fillna(0.0).clip(lower=-0.20, upper=0.20)
