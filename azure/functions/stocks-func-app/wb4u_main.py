...
def findAllBestStocksWithSymbols(FINVIZ_LIST=finviz_options):
  symListAll = []
  symList = []
  symListFull = []
...
def findStocks(symListDupsFull, emailTxt):
     stockList1,stockListFull = findAllBestStocksWithSymbols() #includes Trending stocks
     eFromEtfs,eFromEtfsFull = generateStockListForBestETFs()
     unionList = []
     unionFullList = []
     unionList.extend(stockList1)
     unionList.extend(eFromEtfs)
     unionFullList.extend(stockListFull)
     unionFullList.extend(eFromEtfsFull)
     symListDups = list(set(unionList))
     symListDupsFull = list(set(unionFullList))
     return symListDups,symListDupsFull,emailTxt
...
def createSymbolListFromEquityList(equityList):
    symbolList =  [e.symbol for e in equityList ]
    return symbolList


os.environ['DISABLE_TQDM'] = '1'
symListDups, fulllist,emailTxt = findStocks(symListDupsFull=[],emailTxt='')
