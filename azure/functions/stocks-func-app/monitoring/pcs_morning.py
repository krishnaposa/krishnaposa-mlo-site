...
Scheduled ~10:00 ET (see function_app.pcs_morning). Uses live option chains and
current marks so you can trade the same session.
...
def run_pcs_morning(*, stamp: Optional[str] = None) -> Dict[str, Any]:
    ...
    pcs_opportunities_html: str | None = None
    ...
    if os.getenv("PCS_OPPORTUNITIES_ENABLED", "1") == "1":
        ...
        try:
            ...
            logger.warning("[pcs_morning] opportunities failed: %s", e)
    ...
    pcs_lifecycle_html: str | None = None
    pcs_actionable: list[str] | None = None
    ...
    if os.getenv("PCS_LIFECYCLE_ENABLED", "1") == "1":
        try:
            ...
            logger.warning("[pcs_morning] lifecycle failed: %s", e)
