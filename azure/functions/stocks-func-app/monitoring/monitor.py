...
from .emailer import send_email_report_simple
...
def run_monitor(tickers, *, today=None, min_dollar_vol=MIN_DOLLAR_VOL_DEFAULT):
    ...
    for t, df in frames.items():
        ...
        try: mc = float(mc) if mc is not None else np.nan
        except Exception: mc = np.nan
        if np.isnan(mc):   latest["cap_bonus"] = 0.0
        elif mc < 2e9:     latest["cap_bonus"] = -0.3
        elif mc < 10e9:    latest["cap_bonus"] = 0.1
        elif mc < 200e9:   latest["cap_bonus"] = 0.2
        else:              latest["cap_bonus"] = 0.25
    ...
    from .fundamentals import cap_multiplier as _cap_mult
    ...
    out["cap_mult"]           = out["market_cap"].apply(_cap_mult)
    ...
    def _tickers_only(df):
        ...
        d = df.copy()
        if "ai_score" in d.columns: d = d.sort_values("ai_score", ascending=False)
