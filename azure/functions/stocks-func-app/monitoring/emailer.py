...
def _sim_table(sim_rows):
    if not sim_rows:
        return ""
    rows_html = "".join(
        f"<tr><td>{r['ticker']}</td>"
        f"<td>{r.get('mc_p_up_30d', 0):.1%}</td>"
        f"<td>{r.get('hmm_prob_bull', 0):.1%}</td></tr>"
        for r in sim_rows
    )
    return f"""
    <h3>Simulations Overview</h3>
...
def send_email_report_simple(
    *,
    ...
    """Send HTML summary email with tickers and simulation overview."""
    ...
    sim_html = _sim_table(sim_rows)
    eat_line = f"<p style='color:#666;font-size:13px;'>{eat_note}</p>" if eat_note else ""
