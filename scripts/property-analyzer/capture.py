...
from utils import (log, warn, normalize_hoa, to_float, deep_find_keys,
                   clean_str, dump_json_blob, dump_index, rent_from_payload_url)
...
def fetch_redfin_data_smart(url: str, *, headless=True, slow_mo=0, timeout_ms=45000, dump_blobs=True):
    ...
    for blob in candidates:
        try:
            ...
            if hoa is None:
                from_val = hits.get("hoa") or hits.get("monthlyhoa") or hits.get("hoadues")
                hoa = normalize_hoa(from_val)
