...
function computeSide(prefix) {
  ...
  return { monthlyPmt, cashToClose, fiveYearCost, pointsCost, rate, L, years, monthlyPI: am.monthlyPI };
}

function fmtMoney(v) {
  return isFinite(v) ? v.toLocaleString(undefined, { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }) : '$—';
}
...
function compare() {
  ...
  // Breakeven: rough estimate
  function breakeven(side) {
    if (side.pointsCost <= 0) return 'N/A';
    const alt = amortSummary(side.L, side.rate + 0.0025, side.years, 1).monthlyPI;
    const save = alt - side.monthlyPI;
    return save > 0 ? Math.ceil(side.pointsCost / save) + ' mo' : 'N/A';
  }
  $('resA_breakeven').textContent = breakeven(A);
  $('resB_breakeven').textContent = breakeven(B);
}
